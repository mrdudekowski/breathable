...
import type {
  ...
  ExerciseState,
  BreathingExerciseCallbacks,
  BreathingRoundConfig,
  HoldType,
} from '../types/breathing';
import {
  getPhaseDuration as getPhaseDurationForPhase,
...
const getCurrentRound = (
  practice: BreathingPracticeConfig,
  index?: number
): BreathingRoundConfig | undefined => {
  if (practice.rounds == null || practice.rounds.length === 0 || index == null || index === 0)
    return undefined;
...
const handlePauseCompletion = (
  ...
  if (isLastRound) {
    ...
    const nextRoundIndex = isFirstRound ? 1 : (prevState.currentRoundIndex ?? 1) + 1;
...
const getNextStateOnTick = (
  ...
  if (currentRound && prevState.roundCycle !== undefined && prevState.totalRounds !== undefined) {
    ...
    if (
      prevState.currentPhase === 'pause' &&
      prevState.currentRoundIndex !== undefined &&
      prevState.totalRounds !== undefined
    ) {
    ...
    if (prevState.currentPhase === 'hold') {
      ...
      if (
        prevState.currentHoldType === 'round-exhale' &&
        currentRound != null &&
        currentRound.finalHoldPhase === 'exhale' &&
        practice.globalInhaleHoldDuration !== undefined
      ) {
    ...
    if (
      ...
      !(
        ...
        (prevState.previousHoldType === 'global-inhale' ||
          prevState.previousHoldType === 'round-inhale')
      )
    ) {
    ...
    if (
      ...
      currentRound != null &&
      prevState.roundCycle === currentRound.cycles &&
      currentRound.finalHoldPhase === 'inhale'
    ) {
      const isLastRound = prevState.currentRoundIndex === prevState.totalRounds;
    ...
    if (currentPractice.rounds && currentPractice.rounds.length > 0) {
      ...
      const invalidRound = currentPractice.rounds.find(
        round =>
          !round.cycles ||
          round.cycles <= 0 ||
          !round.finalHoldDuration ||
          round.finalHoldDuration <= 0
      );
      if (invalidRound) {
      ...
      if (prepareFirst) {
        ...
        const initialInhaleDuration = getPhaseDurationWithSpeed(
          'inhale',
          currentPractice,
          firstRound
        );
      ...
      intervalRef.current = window.setInterval(() => {
        setState(prevState => {
          ...
          const {
            ...
            currentPractice,
            phaseOrder,
            isFinalHoldRef.current,
            isGlobalInhaleHoldRef.current
          );
          ...
          if (nextHoldType !== undefined) {
            ...
            prevState.currentPhase === 'exhale' &&
            prevState.previousHoldType !== undefined
          ) {
